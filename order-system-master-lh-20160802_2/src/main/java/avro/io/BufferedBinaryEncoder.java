...
public class BufferedBinaryEncoder extends BinaryEncoder {
    ...
    if (null != this.sink) {
      if ( pos > 0) {
        try {
          ...
          throw new RuntimeException("Failure flushing old output", e);
        }
      }
    }
  ...
  /**writeInt同时返回写入的字节数*/
  public int writeIntWithReturn(int n) throws IOException {
    ensureBounds(5);
    int len = BinaryData.encodeInt(n, buf, pos);
    pos += len;
    return len;
  }
  ...
  /**writeLong同时返回写入的字节数*/
  public int writeLongWithReturn(long n) throws IOException {
    ensureBounds(10);
    int len = BinaryData.encodeLong(n, buf, pos);
    pos += len;
    return len;
  }
